...
<template>
  <NuxtLink :to="linkHref">
    <div class="card" @mouseenter="play" @mouseleave="pause">
      <div class="card__content flex flex-vertical-center">
        <nuxt-icon :name="type" />
        <div>
          <h4>{{ title }}</h4>
          <p>{{ description }}</p>
      ...
      <div class="card__hovered">
        <div ref="wordTrack" class="card__word-track">
          <div v-for="copy in 2" :key="copy" class="card__word-content">
            <span class="card__hovered-text">{{ title }}</span>
